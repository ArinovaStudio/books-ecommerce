  ...
  try {
    ...
    if (!auth.success) {
      return NextResponse.json({
        success: false,
        message: auth.message || "Admin access required",
        status: 403,
      });
    }
    ...
    return NextResponse.json(
      { success: true, count: students.length, students },
      { status: 200 }
    );
  } catch (error) {
...
export const POST = Wrapper(async (req: NextRequest) => {
  try {
        ...
        return NextResponse.json(
          {
            success: false,
            message:
              "Password is required (min 6 chars) for new parent registration",
          },
          { status: 400 }
        );
      }
    ...
    const emailData = studentAddedTemplate(
      ...
      className,
      sectionName,
      isNewParent ? password : undefined
    );
