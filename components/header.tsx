        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden lg:flex items-center gap-6">
            ...
            <div className="flex items-center gap-5">
              {!loading && (
                user ? (
                  <Link href={(user.role === "ADMIN" || user.role === "SUB_ADMIN") ? "/admin" : "/profile"} className="p-2 rounded-full bg-white/5 border border-white/10 text-white hover:bg-cyan-400 hover:text-[#141f38] transition-all" title="Profile">
                    <User size={20} />
                  </Link>
                ) : (
                  <Button
